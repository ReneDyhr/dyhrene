...
**Tools:** Use **`shopping_list_list`** first to read active and checked items (with `id`). **`shopping_list_add_item`** adds a row (name ≥ 3 characters). **`shopping_list_check_item`** / **`shopping_list_uncheck_item`** toggle status. **`shopping_list_remove_item`** deletes a row. **`shopping_list_reorder_items`** sets order for **active** items: pass **`ordered_ids`** as the full top-to-bottom list of active item ids.
